const LESTAT_DIREITA = 1;
const LESTAT_ESQUERDA = 2;
const LESTAT_CIMA = 1;
const LESTAT_BAIXO = 0;
const LESTAT_PARADO = 0;
...
class Lestat {
  ...
  atualizar() {
    ...
    if (this.teclado.pressionada(SETA_DIREITA)) {
      if (!this.andando || this.direcao !== LESTAT_DIREITA) {
        this.sheet.linha = 3;
        this.sheet.coluna = 0;
      }
      ...
      if (!this.andando || this.direcao !== LESTAT_ESQUERDA) {
        this.sheet.linha = 2;
        this.sheet.coluna = 0;
      }

      this.andando = true;
      this.direcao = LESTAT_ESQUERDA;
      this.sheet.proximoQuadro();
      this.x -= this.velocidade;
      this.atacando = false;
    }
    ...
    // Ataque
      ...
      if (this.direcao === LESTAT_ESQUERDA) {
        this.sheetAtaque.linha = 2; // linha para ataque à esquerda
      } else if (this.direcao === LESTAT_DIREITA) {
  ...
  retangulosColisao() {
    ...
    let frameDoGolpe;
    if (this.direcao === LESTAT_DIREITA) {
      frameDoGolpe = 5; // frame de ataque para a direita
    } else if (this.direcao === LESTAT_ESQUERDA) {
  ...
  colidiuCom() {
    // Quando o golpe do LESTAT atingir o vilão
