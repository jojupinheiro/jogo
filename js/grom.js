const GROM_DIREITA = 1;
const GROM_ESQUERDA = 2;
const GROM_CIMA = 1;
const GROM_BAIXO = 0;
const GROM_PARADO = 0;
...
class Grom {
  ...
  atualizar() {
    ...
    if (this.teclado.pressionada(SETA_DIREITA)) {
      if (!this.andando || this.direcao !== GROM_DIREITA) {
        this.sheet.linha = 3;
        this.sheet.coluna = 0;
      }
      ...
      if (!this.andando || this.direcao !== GROM_ESQUERDA) {
        this.sheet.linha = 2;
        this.sheet.coluna = 0;
      }

      this.andando = true;
      this.direcao = GROM_ESQUERDA;
      this.sheet.proximoQuadro();
      this.x -= this.velocidade;
      this.atacando = false;
    }
    //Para andar para cima
    else if (this.teclado.pressionada(SETA_CIMA)){
      if(!this.andando || this.direcao !== GROM_CIMA){
        this.sheet.linha = 1;
        this.sheet.coluna = 0;
      }

      this.andando = true;
      this.direcao = GROM_CIMA;
    ...
    //Para andar para baixo
    else if(this.teclado.pressionada(SETA_BAIXO)){
      if(!this.andando || this.direcao !== GROM_BAIXO){
        this.sheet.linha = 0;
        this.sheet.coluna = 0;
      }

      this.andando = true;
      this.direcao = GROM_BAIXO;
    ...
    // Ataque
      ...
      if (this.direcao === GROM_ESQUERDA) {
        this.sheetAtaque.linha = 2; // linha para ataque à esquerda
      } else if (this.direcao === GROM_DIREITA) {
  ...
  retangulosColisao() {
    ...
    let frameDoGolpe;
    if (this.direcao === GROM_DIREITA) {
      frameDoGolpe = 5; // frame de ataque para a direita
    } else if (this.direcao === GROM_ESQUERDA) {
  ...
  colidiuCom() {
    // Quando o golpe do personagem atingir o vilão
